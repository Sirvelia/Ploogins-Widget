...
<template>
    <div class="ploogins:rounded-lg ploogins:p-4 ploogins:flex ploogins:gap-4 ploogins:justify-between ploogins:border-2 ploogins:min-h-[11.375rem]"
        ...
        <div class="ploogins:grow ploogins:flex ploogins:flex-col ploogins:gap-2">
            <div class="ploogins:flex ploogins:gap-4">
                ...
                <div class="ploogins:flex ploogins:flex-col ploogins:gap-2">
                    <div>
                        <p class="ploogins:text-2xl ploogins:font-bold" v-html="result.name"></p>
                        <p class="ploogins:text-sm ploogins:[&_a]:hover:underline" v-html="`by ${result.author}`"></p>
                    </div>

                    <p class="ploogins:line-clamp-2" v-html="strip_tags(result.description, ['iframe'])"></p>
                </div>
            </div>

            <div class="ploogins:flex ploogins:gap-2">
                <div v-for="tag in Object.values(result.tags)"
                    class="ploogins:border ploogins:rounded-full ploogins:px-4 ploogins:py-1 ploogins:text-sm ploogins:line-clamp-1"
                    :style="{ borderColor: borderColor }">{{ tag }}
                </div>
            </div>
        </div>
        ...
        <div class="ploogins:flex ploogins:flex-col ploogins:justify-between ploogins:min-w-fit">
            <div v-if="result.business_model !== 'premium'" class="ploogins:flex ploogins:gap-4">
                <div class="ploogins:flex ploogins:items-center ploogins:gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                        ...
                        class="lucide lucide-star w-4 h-4 ploogins:fill-yellow-400 ploogins:text-yellow-400">
                        <polygon
            ...
            <a :href="result.business_model === 'premium' ? result.homepage : `https://wordpress.org/plugins/${result.slug}`"
                target="_blank"
                class="ploogins:flex ploogins:gap-2 ploogins:items-center ploogins:text-sm ploogins:font-bold ploogins:cursor-pointer ploogins:border-2 ploogins:rounded-lg ploogins:px-4 ploogins:py-2 ploogins:transition-all ploogins:duration-300 ploogins:hover:opacity-70"
                :style="{ backgroundColor: color, color: textColor, borderColor: borderColor }">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                    stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
